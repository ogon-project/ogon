...
#define ICP_CLIENT_STUB_SETUP(camel, expanded)                      \
	...
	pbRPCPayload *pbresponse = nullptr;                             \
	size_t ret;                                                     \
	Ogon__Icp__##camel##Request request;                            \
	Ogon__Icp__##camel##Response *response = nullptr;               \
...
#define ICP_CLIENT_STUB_CALL(camel, expanded)                               \
	...
	if (!(pbrequest.data = static_cast<char *>(malloc(pbrequest.dataLen)))) \
		return PBRPC_FAILED;                                                \
...
#define ICP_CLIENT_STUB_CALL_ASYNC(camel, expanded)                         \
	...
	if (ret == pbrequest.dataLen) {                                         \
		pbrcp_call_method_async(                                            \
				context, type, &pbrequest, dummyCallback, (void *)nullptr); \
	}                                                                       \
	free(pbrequest.data);

#define ICP_CLIENT_STUB_UNPACK_RESPONSE(camel, expanded)                \
	response = ogon__icp__##expanded##_response__unpack(                \
			nullptr, pbresponse->dataLen, (uint8_t *)pbresponse->data); \
	pbrpc_free_payload(pbresponse);

#define ICP_CLIENT_STUB_CLEANUP(camel, expanded) \
	if (response)                                \
		ogon__icp__##expanded##_response__free_unpacked(response, nullptr);
		...
		if (status == 0) {                                             \
			pbresponse->dataLen =                                      \
					ogon__icp__##expanded##_response__get_packed_size( \
							&response);                                \
			if (!(pbresponse->data = static_cast<char *>(              \
						  malloc(pbresponse->dataLen)))) {             \
				return -1;                                             \
			}                                                          \
			...
			pbresponse->data = nullptr;                                \
			ret = 0;                                                   \
		}                                                              \
	} while (0)

int ogon_icp_sendResponse(UINT32 tag, UINT32 type, UINT32 status, BOOL success,
		void *responseparam1) {
	int rtype = 0;
	...
	switch (type) {
		...
		case NOTIFY_STOP_SHADOWING: {
			...
			response.success = success;
			ICP_CLIENT_SEND_PACK(
					OtsApiStartRemoteControl, ots_api_stop_remote_control);
			break;
		}
...
int ogon_icp_DisconnectUserSession(UINT32 connectionId, BOOL *disconnected) {
	...
	ICP_CLIENT_STUB_UNPACK_RESPONSE(
			DisconnectUserSession, disconnect_user_session)
...
int ogon_icp_LogonUser(UINT32 connectionId, const char *username,
		const char *domain, const char *password, const char *clientHostName,
		const char *clientAddress, UINT32 clientBuild, UINT16 clientProductId,
		UINT32 hardwareID, UINT16 protocol, UINT32 width, UINT32 height,
		UINT32 bbp, ogon_backend_props *props, UINT32 *maxWidth,
		UINT32 *maxHeight) {
...
#endif	//  __clang__ || (__GNUC__ > 4 || (__GNUC__ == 4 && __GNUC_MINOR__ >=
		//  6))
...
int ogon_icp_get_property_number(
		UINT32 connectionId, char *path, INT32 *value) {
	if (value == nullptr) return -1;
...
int ogon_icp_get_property_string(
		UINT32 connectionId, char *path, char **value) {
	if (value == nullptr) return -1;
...
static int ogon_icp_get_property_bulk_fallback(
		UINT32 connectionId, PropertyItem *items) {
	PropertyItem *pItem;
	...
	for (pItem = items; pItem->path; pItem++) {
		switch (pItem->propertyType) {
			case PROPERTY_BOOL:
				status = ogon_icp_get_property_bool(
						connectionId, pItem->path, &pItem->v.boolValue);
				break;
			case PROPERTY_NUMBER:
				status = ogon_icp_get_property_number(
						connectionId, pItem->path, &pItem->v.intValue);
				break;
			case PROPERTY_STRING:
				status = ogon_icp_get_property_string(
						connectionId, pItem->path, &pItem->v.stringValue);
				break;
		}
		...
		if (!pItem->success) {
			WLog_ERR(TAG,
					"bulkProperty fallback: error retrieving property "
					"%s(type=%d) status=%d",
					pItem->path, pItem->propertyType, status);
		} else {
			nbSuccess++;
		}
	}

	/* if we had no success ans some fails, return the last status that may be
	 * relevant */
...
void ogon_PropertyItem_free(PropertyItem *items) {
	for (; items->path; items++) {
		if (items->propertyType == PROPERTY_STRING) free(items->v.stringValue);
	}
}

#define MAX_PROPERTIES_NB 30
static int real_ogon_icp_get_property_bulk(
		UINT32 connectionId, PropertyItem *items) {
	size_t i, nitems = 0;
	...
	for (i = 0, propertyItem = items;
			propertyItem->path && i < MAX_PROPERTIES_NB; propertyItem++, i++) {
		...
		switch (propertyItem->propertyType) {
			case PROPERTY_BOOL:
				protobufReq->propertytype =
						OGON__ICP__ENUM_PROPERTY_TYPE__PROP_BOOL;
				break;
			case PROPERTY_NUMBER:
				protobufReq->propertytype =
						OGON__ICP__ENUM_PROPERTY_TYPE__PROP_NUMBER;
				break;
			case PROPERTY_STRING:
				protobufReq->propertytype =
						OGON__ICP__ENUM_PROPERTY_TYPE__PROP_STRING;
				break;
			default:
				break;
		}
	...
	for (i = 0; i < nitems; propertyItem++, i++) {
		...
		if (!propertyItem->success) continue;
		...
		switch (propertyItem->propertyType) {
			...
			case PROPERTY_STRING:
				if (!protobufValue->stringvalue)
					...
					propertyItem->v.stringValue =
							_strdup(protobufValue->stringvalue);
					if (!propertyItem->v.stringValue) {
						WLog_ERR(TAG,
								"unable to strdup a string property result");
						propertyItem->success = false;
					}
				}
				break;
...
int ogon_icp_get_property_bulk(UINT32 connectionId, PropertyItem *items) {
	...
	if (!ogon_icp_get_protocol_version(icp, &vmajor, &vminor)) {
		WLog_ERR(TAG,
				"ogon_icp_get_property_bulk: pbrpc transport not connected");
