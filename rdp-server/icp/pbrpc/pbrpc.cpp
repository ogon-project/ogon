...
static void list_dictionary_item_free(void *value) { free(value); }

pbRPCContext *pbrpc_server_new(
		pbRPCTransportContext *transport, HANDLE shutdown) {
	auto *context = new pbRPCContext;
	wObject *obj;
...
static int pbrpc_receive_message(
		pbRPCContext *context, char **msg, int *msgLen) {
	...
	*msgLen = len;
	if (!(recvbuffer = static_cast<char *>(malloc(len)))) {
		return -1;
	}
...
static int pbrpc_send_message(
		pbRPCContext *context, const char *msg, UINT32 msgLen) {
	UINT32 msgLenWire;
	int ret;
...
static int pbrpc_process_response(
		pbRPCContext *context, Ogon__Pbrpc__RPCBase *rpcmessage) {
	pbRPCTransaction *ta;

	if (!(ta = static_cast<pbRPCTransaction *>(
				  ListDictionary_Remove(context->transactions,
						  (void *)((UINT_PTR)rpcmessage->tag))))) {
		WLog_ERR(TAG, "Unsolicited response - ignoring (tag %" PRIu32 ")",
				rpcmessage->tag);
		ogon__pbrpc__rpcbase__free_unpacked(rpcmessage, nullptr);
		return 1;
	}
...
static int pbrpc_process_message_out(
		pbRPCContext *context, Ogon__Pbrpc__RPCBase *msg) {
	int ret;
	int msgLen = ogon__pbrpc__rpcbase__get_packed_size(msg);
	std::string buf;
	buf.reserve(msgLen);

	ret = ogon__pbrpc__rpcbase__pack(
			msg, reinterpret_cast<uint8_t *>(buf.data()));
	DEBUG_PBRPC("sending tag %" PRIu32 ", type %" PRIu32 ", response: %d",
			msg->tag, msg->msgtype, msg->isresponse);
	// packing failed..
	if (ret != msgLen) {
		ret = 1;
	} else {
		ret = pbrpc_send_message(context, buf.c_str(), msgLen);
	}
...
static Ogon__Pbrpc__RPCBase *pbrpc_response_new(
		pbRPCPayload *response, UINT32 status, UINT32 type, UINT32 tag) {
	...
	pbresponse->status = static_cast<Ogon__Pbrpc__RPCBase__RPCSTATUS>(status);
...
static int pbrpc_send_response(pbRPCContext *context, pbRPCPayload *response,
		UINT32 status, UINT32 type, UINT32 tag) {
	int ret;
	Ogon__Pbrpc__RPCBase *pbresponse;
...
int pbrpc_respond_method(pbRPCContext *context, pbRPCPayload *response,
		UINT32 status, UINT32 type, UINT32 tag) {
	Ogon__Pbrpc__RPCBase *pbresponse;
...
static int pbrpc_process_request(
		pbRPCContext *context, Ogon__Pbrpc__RPCBase *rpcmessage) {
	int ret = 0;
	pbRPCCallback cb;
	pbRPCPayload *request = nullptr;
	pbRPCPayload *response = nullptr;

	if (!(cb = pbrpc_callback_find(context, rpcmessage->msgtype))) {
		WLog_ERR(TAG, "server callback not found %" PRIu32 "",
				rpcmessage->msgtype);
		ret = pbrpc_send_response(context, nullptr,
				OGON__PBRPC__RPCBASE__RPCSTATUS__NOTFOUND, rpcmessage->msgtype,
				rpcmessage->tag);
		ogon__pbrpc__rpcbase__free_unpacked(rpcmessage, nullptr);
		return ret;
	}
	...
	if (!(request = pbrpc_fill_payload(rpcmessage))) {
		...
		ret = pbrpc_send_response(context, nullptr,
				OGON__PBRPC__RPCBASE__RPCSTATUS__FAILED, rpcmessage->msgtype,
				rpcmessage->tag);
		ogon__pbrpc__rpcbase__free_unpacked(rpcmessage, nullptr);
		return ret;
	}
	...
	ret = pbrpc_send_response(
			context, response, ret, rpcmessage->msgtype, rpcmessage->tag);
	ogon__pbrpc__rpcbase__free_unpacked(rpcmessage, nullptr);
	return ret;
}
...
static int pbrpc_process_message_in(pbRPCContext *context) {
	...
	rpcmessage = ogon__pbrpc__rpcbase__unpack(nullptr, msgLen, (uint8_t *)msg);
	...
	DEBUG_PBRPC("received tag %" PRIu32 " size %" PRIuz ", type %" PRIu32
				" status %d, response %d",
			rpcmessage->tag, rpcmessage->payload.len, rpcmessage->msgtype,
			rpcmessage->status, rpcmessage->isresponse);
...
static BOOL pbrpc_connect(pbRPCContext *context, DWORD timeout) {
	...
	if (!(message = pbrpc_message_new())) {
		WLog_ERR(TAG,
				"error while creating a new pbrpc message, disconnecting!");
		goto error_out;
	}
	...
	message = ogon__pbrpc__rpcbase__unpack(nullptr, msgLen, (uint8_t *)msg);
	free(msg);
	...
	if (message->versioninfo == nullptr) {
	...
	if (message->versioninfo->vmajor != OGON_PROTOCOL_VERSION_MAJOR) {
		WLog_ERR(TAG,
				"error, ogon-session-manager (protocol %" PRIu32 ".%" PRIu32
				") is not compatible with ogon(%u.%u)!",
	...
	WLog_DBG(TAG,
			"Version information received from session-manager: %" PRIu32
			".%" PRIu32 "",
			message->versioninfo->vmajor, message->versioninfo->vminor);
	context->remoteVersionMajor = message->versioninfo->vmajor;
	context->remoteVersionMinor = message->versioninfo->vminor;

	ogon__pbrpc__rpcbase__free_unpacked(message, nullptr);
...
static BOOL pbrpc_reconnect(pbRPCContext *context) {
	...
	while ((ta = static_cast<pbRPCTransaction *>(
					ListDictionary_Remove_Head(context->transactions)))) {
		ta->responseCallback(PBRCP_TRANSPORT_ERROR, nullptr, ta->callbackArg);
		free(ta);
	}
...
static void pbrpc_mainloop(pbRPCContext *context) {
	...
	sigset_t set;
	sigfillset(&set);
	status = pthread_sigmask(SIG_BLOCK, &set, nullptr);
	if (0 != status)
	...
	while (context->runLoop) {
		if (reconnect) {
			if (!firstLoop) {
				if (context->disconnectedCb) context->disconnectedCb();
			...
			firstLoop = FALSE;
			if (!pbrpc_reconnect(context)) continue;
			reconnect = FALSE;
		}
		...
		if (WaitForSingleObject(Queue_Event(context->writeQueue), 0) ==
				WAIT_OBJECT_0) {
			Ogon__Pbrpc__RPCBase *msg = nullptr;

			while ((msg = static_cast<Ogon__Pbrpc__RPCBase *>(
							Queue_Dequeue(context->writeQueue))) &&
					context->runLoop) {
...
int pbrpc_server_start(pbRPCContext *context, UINT32 vmajor, UINT32 vminor) {
	...
	context->thread = CreateThread(nullptr, 0,
			(LPTHREAD_START_ROUTINE)pbrpc_mainloop, context, 0, nullptr);
...
static void pbrpc_response_local_cb(
		UINT32 reason, Ogon__Pbrpc__RPCBase *response, void *args) {
	struct pbrpc_local_call_context *context =
			(struct pbrpc_local_call_context *)args;
	context->response = response;
	context->status = static_cast<PBRPCSTATUS>(reason);
	SetEvent(context->event);
}

int pbrpc_call_method(pbRPCContext *context, UINT32 type, pbRPCPayload *request,
		pbRPCPayload **response) {
	Ogon__Pbrpc__RPCBase *message;
	pbRPCTransaction *ta;
	UINT32 ret;
	UINT32 tag;
	...
	if (!(local_context.event = CreateEvent(nullptr, TRUE, FALSE, nullptr))) {
	...
	if (!ListDictionary_Add(context->transactions,
				(void *)((UINT_PTR)(message->tag)), ta)) {
	...
	if (wait_ret != WAIT_OBJECT_0) {
		pbRPCTransaction *fa;
		if (!(fa = static_cast<pbRPCTransaction *>(ListDictionary_Remove(
					  context->transactions, (void *)((UINT_PTR)(tag)))))) {
			/**
			 * timeout occurred but request is being processed by the pbrpc
			 * thread, wait for the event to be notified so that we can safely
			 * close the Event handle (the pbRPCTransaction fa will be freed by
			 * the pbrpc thread).
			 */
...
fail_queue_enqueue:
	ListDictionary_Remove(
			context->transactions, (void *)((UINT_PTR)(message->tag)));
fail_dictionary_add:
	free(ta);
...
void pbrcp_call_method_async(pbRPCContext *context, UINT32 type,
		pbRPCPayload *request, pbRpcResponseCallback callback,
		void *callback_args) {
	Ogon__Pbrpc__RPCBase *message;
	pbRPCTransaction *ta;
...
fail_transaction_new:
	free(payloadData);
fail:
	callback(err, nullptr, callback_args);
	return;
}
