...
#define OGON_DMGBUF_POS_PADDING       (OGON_DMGBUF_POS_RECTS + OGON_DMGBUF_MAX_RECTS * sizeof(RDP_RECT))
#define OGON_DMGBUF_POS_DATA          OGON_DMGBUF_ALIGN(OGON_DMGBUF_POS_PADDING, OGON_DMGBUF_DATA_ALIGNMENT)

#define OGON_DMGBUF_MEM_SIZE(h,s)     (OGON_DMGBUF_POS_DATA+h*s)

#define OGON_DMGBUF_PTR_MAGIC(p)      ((UINT32*)(((char*)p)+OGON_DMGBUF_POS_MAGIC))
#define OGON_DMGBUF_PTR_SHMID(p)      ((INT32*)(((char*)p)+OGON_DMGBUF_POS_SHMID))
#define OGON_DMGBUF_PTR_WIDTH(p)      ((UINT32*)(((char*)p)+OGON_DMGBUF_POS_WIDTH))
#define OGON_DMGBUF_PTR_HEIGHT(p)     ((UINT32*)(((char*)p)+OGON_DMGBUF_POS_HEIGHT))
#define OGON_DMGBUF_PTR_SCANLINE(p)   ((UINT32*)(((char*)p)+OGON_DMGBUF_POS_SCANLINE))
#define OGON_DMGBUF_PTR_MAX_RECTS(p)  ((UINT32*)(((char*)p)+OGON_DMGBUF_POS_MAX_RECTS))
#define OGON_DMGBUF_PTR_NUM_RECTS(p)  ((UINT32*)(((char*)p)+OGON_DMGBUF_POS_NUM_RECTS))
#define OGON_DMGBUF_PTR_RECTS(p)      ((RDP_RECT*)(((char*)p)+OGON_DMGBUF_POS_RECTS))
#define OGON_DMGBUF_PTR_DATA(p)       ((BYTE*)(((char*)p)+OGON_DMGBUF_POS_DATA))

#define OGON_DMGBUF_GOOD(p)           (p && *OGON_DMGBUF_PTR_MAGIC(p)==OGON_DMGBUF_MAGIC)
...
int ogon_dmgbuf_set_user(void *handle, unsigned int user_id) {
	...
	if (shmctl(*OGON_DMGBUF_PTR_SHMID(handle), IPC_STAT, &buf) < 0) {
		WLog_ERR(TAG, "shmctl IPC_STAT failed, error=%s(%d)", strerror(errno), errno);
		return -1;
	}
	...
	if (buf.shm_perm.uid != user_id) {
		...
		if (shmctl(*OGON_DMGBUF_PTR_SHMID(handle), IPC_SET, &buf) < 0) {
			WLog_ERR(TAG, "shmctl IPC_SET failed, error=%s(%d)", strerror(errno), errno);
			return -1;
		}
	}
	return 0;
...
void* ogon_dmgbuf_new(int width, int height, int scanline) {
	...
	if (width < 1 || height < 1 || scanline < width * 4) {
		WLog_ERR(TAG, "invalid parameters: width=%d height=%d scanline=%d", width, height, scanline);
		return NULL;
	}
	...
	if (shmctl(segment_id, IPC_RMID, 0) < 0) {
		WLog_ERR(TAG, "error unreferencing SHM segment %d, error=%s(%d)", segment_id, strerror(errno), errno);
		return NULL;
	}
	...
	*OGON_DMGBUF_PTR_MAX_RECTS(handle) = OGON_DMGBUF_MAX_RECTS;
	...
	if ((unsigned long)OGON_DMGBUF_PTR_DATA(handle) % OGON_DMGBUF_DATA_ALIGNMENT) {
		WLog_ERR(TAG, "internal error: damage buffer data is not correctly aligned!");
		return NULL;
	}
...
void* ogon_dmgbuf_connect(int buffer_id) {
	...
	if (handle == ((void*) (size_t) (-1))) {
		WLog_ERR(TAG, "shmat(%d) failed, error=%s(%d)",
		         buffer_id, strerror(errno), errno);
		return NULL;
	}

	WLog_DBG(TAG, "attached to an existing shared memory segment 0x%08X", buffer_id);
	if (!OGON_DMGBUF_GOOD(handle)) {
